...
void C3DGraph::RenderGraph()
{
	...
	for (m = 0; m < MAX_3D_EQS; ++m) {
		if ((m_3DEquations[m].wires == CW || m_3DEquations[m].wires == W)
			&& !m_bFailedEquationGeneration && m_3DEquations[m].on) {
...
void C3DGraph::BuildList(int eq)
{
	...
	*********************/
	...
	if (m_3DEquations[eq].wires == CWW) m_3DEquations[eq].wires = CW;
	...
	m_pCalculator->SetErrorMode(ERROR_MODE_TRANSLATE);
	if (m_pCalculator->Compute3D(m_3DEquations[eq].eq, &mmvariables, m_z_max, 0) != SUCCESSFULL) {
		m_pCalculator->SetErrorMode(ERROR_MODE_COMPLETE);
	...
	if (m_3DEquations[eq].wires != C) {
		for (x = m_x_min, x_count = 0; x_count < m_x_interval; ++x_count, x += x_step) {
			//if (fabs(m_origin_x - x) > 0.0000001){ //Make sure we aren't on top of the axis
		...
		for (y = m_y_min, y_count = 0; y_count < m_y_interval; ++y_count, y += y_step, ++m_master_count) {
			//if (fabs(m_origin_y - y) > 0.0000001){ //Make sure we aren't on the axis
			glBegin(GL_LINE_STRIP);
			glColor3f(0, 0, 0);
			for (x = m_x_min, x_count = 0; x_count <= m_x_interval; ++x_count, x += x_step) {
				if (results_x[x_count].results_y[y_count] != HUGE_VAL) {
...
void C3DGraph::ExportDXF(CString filename)
{
	...
	for (int n = 0; n < 6; ++n) { // loop for each equation
		if (m_3DEquations[n].on) {
			...
			m_pCalculator->SetErrorMode(ERROR_MODE_TRANSLATE);
			if (m_pCalculator->Compute3D(m_3DEquations[n].eq, &mmvariables, m_z_max, 0) != SUCCESSFULL)	m_pCalculator->SetErrorMode(ERROR_MODE_COMPLETE);
			m_pCalculator->SetErrorMode(ERROR_MODE_COMPLETE);
...
BOOL C3DGraph::OnInitDialog()
{
	...
	CMenu PopUp3DGraph, PopUpView;
	PopUp3DGraph.LoadMenu(IDR_3DGRAPH);
	PopUpView.LoadMenu(IDR_3DGRAPH_VIEW);
	...
	m_Menu3DGraph.InsertMenu(6, MF_BYPOSITION | MF_POPUP | MF_ENABLED, (UINT)PopUpView.m_hMenu, "View");
