...
BOOL TFXDataTip::PreTranslateMessage(MSG* pMsg)
{
	...
	case WM_CANCELMODE:
		Hide();
		POINT lp;
		lp.x = (LONG)(short)LOWORD(pMsg->lParam);
		lp.y = (LONG)(short)HIWORD(pMsg->lParam);
		ClientToScreen(&lp);
		m_parent->ScreenToClient(&lp);
		pMsg->lParam = MAKELONG(lp.x, lp.y);
		m_parent->SendMessage(pMsg->message, pMsg->wParam, pMsg->lParam);
